...
function Shell({ user, logout }) {
  ...
  const visibleNav = useMemo(() => NAV.filter(([id]) => !['platform', 'iam', 'security'].includes(id) && hasAccess(id)), [hasAccess])
...
function IAM({ notice, currentUser }) {
  ...
  return <><PageHead eyebrow="Identity & access management" title="IAM" action={<Button icon={Plus} onClick={reset}>New user</Button>} />
    <Card className="security-banner"><span><UserCog/></span><div><h2>Internal platform access</h2><p>Customer registrations are created automatically and appear under Customer accounts. IAM is reserved for your internal administrator and support identities.</p></div></Card>
