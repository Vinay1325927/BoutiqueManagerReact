...
const fileSize = (value) => { const bytes=Number(value||0);if(bytes<1024)return `${bytes} B`;if(bytes<1024**2)return `${(bytes/1024).toFixed(1)} KB`;if(bytes<1024**3)return `${(bytes/1024**2).toFixed(1)} MB`;return `${(bytes/1024**3).toFixed(2)} GB` }
const dateLabel = (value) => { try { return format(parseISO(String(value).slice(0, 10)), 'dd MMM yyyy') } catch { return value || '—' } }

const NAV = [
  ['platform', 'Customer accounts', Building2],
...
const FEATURE_OPTIONS = NAV.filter(([id]) => !['platform', 'iam', 'security', 'technical', 'backup'].includes(id)).map(([id, label]) => ({ id, label }))
const VIEWER_FEATURES = ['dashboard', 'review', 'customers', 'vendors', 'analytics', 'reminders', 'bill', 'notes', 'ai']
const ROLE_LABELS = { admin: 'Platform administrator', owner: 'Workspace owner', custom: 'Custom access', viewer: 'Viewer · read only' }
...
const LANDING_FEATURES = [
  ...
  [UserCog, 'Private workspace', 'Every signup receives an isolated workspace with secure password, Google, Microsoft and optional PEM login.'],
  [Bot, 'AI assistant', 'Ask questions about accounts, collections, vendors, work notes and daily priorities.'],
  [ArchiveRestore, 'Protected records', 'Business records stay private to each workspace while the platform owner can monitor account health.'],
]
...
function PublicHome({ onLogin }) {
  ...
  return <main className="public-page landing-page">
    <header className="public-nav landing-nav"><a className="brand-lockup" href="#top"><span className="brand-mark"><Store/></span><div><strong>Boutique Cloud</strong><span>Business workspace</span></div></a><nav><a href="#features">Features</a><a href="#platform">Technology</a><a href="#security">Security</a></nav><div className="landing-auth"><Button variant="ghost" icon={LogIn} onClick={() => setLoginOpen(true)}>Log in</Button><Button icon={UserPlus} onClick={openSignup}>Sign up</Button></div></header>
    <div className="landing-notice"><Notice value={notice} clear={()=>setNotice(null)}/></div>
    <section className="landing-hero" id="top"><div className="landing-copy"><span className="landing-kicker"><Sparkles/>A complete boutique operations workspace</span><h1>Run every boutique account from <em>one calm workspace.</em></h1><p>Manage sales, customers, collections, bills, passbook records, vendors and analytics without scattered spreadsheets or manual follow-ups.</p><div className="landing-actions"><Button icon={Rocket} onClick={openSignup}>Create free workspace</Button><Button variant="secondary" icon={LogIn} onClick={() => setLoginOpen(true)}>Log in</Button></div><div className="landing-trust"><span><CheckCircle2/>Instant signup</span><span><CheckCircle2/>Isolated MongoDB records</span><span><CheckCircle2/>Secure authentication</span></div></div><div className="product-window"><div className="product-window-top"><div><span/><span/><span/></div><small>Business overview</small><span className="badge green">Live workspace</span></div><div className="product-metrics"><div><span>Revenue</span><strong>₹56,395</strong><small>Sales performance</small></div><div><span>Pending</span><strong>₹41,486</strong><small>Collection queue</small></div><div><span>Customers</span><strong>128</strong><small>Relationship records</small></div></div><div className="product-body"><div className="product-chart"><div className="chart-heading"><span>Monthly revenue</span><small>Revenue · Profit</small></div><div className="chart-bars">{[38,62,48,82,57,74,92,68].map((height,index)=><i key={index} style={{height:`${height}%`}}/>)}</div></div><div className="product-activity"><span>Today’s priorities</span>{[['Collect pending payment','₹8,500'],['Generate customer bill','PDF'],['Review passbook entries','12 rows']].map(([label,value])=><div key={label}><i/><p>{label}<small>Ready for action</small></p><strong>{value}</strong></div>)}</div></div></div></section>
    ...
    <section className="landing-section security-section" id="security"><div className="security-showcase"><span><ShieldCheck/></span><div><span className="eyebrow">Security by design</span><h2>Your business data belongs to your workspace.</h2><p>Each signup creates a separate account boundary. Server-side checks isolate business records, social login uses verified provider identities, and optional PEM sign-in keeps the private key on the user’s device.</p></div></div><div className="security-cards"><article><Building2/><strong>Private workspace</strong><p>Customers cannot view or change another organisation’s records.</p></article><article><LockKeyhole/><strong>Secure identity</strong><p>Password, Google, Microsoft and PEM sign-in are verified by the server.</p></article><article><Eye/><strong>Read-only controls</strong><p>Viewer access is protected with server-side write enforcement.</p></article></div></section>
    <section className="landing-cta"><span className="eyebrow">Ready to get organised?</span><h2>Create your private business workspace instantly.</h2><p>Sign up and start managing sales, customers, bills and accounts immediately. No approval or waiting period.</p><div><Button icon={UserPlus} onClick={openSignup}>Create free workspace</Button><Button variant="ghost" onClick={() => setLoginOpen(true)}>Already registered? Log in</Button></div></section>
    <footer className="landing-footer"><div className="brand-lockup"><span className="brand-mark"><Store/></span><div><strong>Boutique Cloud</strong><span>Business workspace</span></div></div><p>Sales, accounts, billing and secure business data in one private workspace.</p><span>React · Express · MongoDB · Python</span></footer>
    {loginOpen && <LoginModal close={() => setLoginOpen(false)} onLogin={onLogin} />}
    {signupOpen && <SignupModal oauthContext={oauthContext} onLogin={onLogin} close={() => { setSignupOpen(false); setOauthContext(null) }} openLogin={() => { setSignupOpen(false); setOauthContext(null); setLoginOpen(true) }} />}
  </main>
}
...
function SignupModal({ close, openLogin, oauthContext, onLogin }) {
  ...
  return <div className="modal-backdrop" onMouseDown={close}>
    <div className="modal signup-modal" onMouseDown={(e)=>e.stopPropagation()}>
      ...
      {complete ? <div className="signup-complete">
        ...
        <p>Your private workspace is ready. You can now log in and start managing your business.</p>
        <div><Button icon={LogIn} onClick={openLogin}>Go to login</Button><Button variant="ghost" onClick={close}>Close</Button></div>
      </div> : <>
        <div className="signup-head"><span className="login-mark"><Building2/></span><div><span className="eyebrow">Create your private workspace</span><h2>{oauthContext ? 'A few details before we open your workspace.' : 'Tell us about you and your organisation.'}</h2><p>{oauthContext ? `Your ${oauthContext.provider} identity is verified. Complete the remaining questions and you will enter the app immediately.` : 'Continue with Google or Microsoft first, or complete the form with a workspace password.'}</p></div></div>
        {oauthContext ? <div className="oauth-connected"><span className={oauthContext.provider === 'google' ? 'oauth-google' : 'oauth-microsoft'}>{oauthContext.provider === 'google' ? 'G' : <><i/><i/><i/><i/></>}</span><div><strong>{oauthContext.email}</strong><small>Verified with {oauthContext.provider}. No additional password is required.</small></div><CheckCircle2/></div> : <div className="signup-first"><div><strong>Continue with an existing identity</strong><small>We will sign you in when the identity already has an account, or return here with your name and email verified to create a workspace instantly.</small></div><div className="oauth-buttons"><button type="button" onClick={()=>socialSignup('google')}><span className="oauth-google">G</span>Continue with Google</button><button type="button" onClick={()=>socialSignup('microsoft')}><span className="oauth-microsoft"><i/><i/><i/><i/></span>Continue with Microsoft</button></div><div className="auth-divider"><span>or answer below and create a password</span></div></div>}
        <form onSubmit={submit}>
          ...
          <div className="signup-section">
            ...
            {oauthContext ? <div className="social-security-note"><ShieldCheck/><span><strong>Provider-protected account</strong><small>You will use Continue with {oauthContext.provider} to enter your workspace.</small></span></div> : <div className="form-grid two"><Field label="Password" type="password" minLength="8" value={form.password} onChange={(e)=>set('password',e.target.value)} required/><Field label="Confirm password" type="password" minLength="8" value={form.confirm_password} onChange={(e)=>set('confirm_password',e.target.value)} required/></div>}
            <label className="signup-consent"><input type="checkbox" checked={form.terms} onChange={(e)=>set('terms',e.target.checked)}/><span>I confirm these details are correct and agree to create a private organisation workspace.</span></label>
          </div>
          {error&&<p className="form-error">{error}</p>}
          <div className="signup-actions"><p><ShieldCheck/>{oauthContext ? 'Your verified identity will be linked to this private workspace.' : 'Passwords are hashed before storage.'}</p><Button icon={UserPlus} disabled={busy}>{busy?'Creating workspace…':'Create my workspace'}</Button></div>
        </form>
      </>}
    </div>
  </div>
...
function LoginModal({ close, onLogin }) {
  const [form, setForm] = useState({ username: '', password: '' }), [mode, setMode] = useState('password'), [pemFile, setPemFile] = useState(null)
...
function PlatformConsole({ user, logout }) {
  const [theme, setTheme] = useState(localStorage.getItem('boutique_theme') || 'light'), [notice, setNotice] = useState(null)
  useEffect(() => { document.documentElement.dataset.theme = theme; localStorage.setItem('boutique_theme', theme) }, [theme])
  return <div className="platform-console-shell">
    <header className="platform-console-top"><div className="brand-lockup"><span className="brand-mark"><Building2/></span><div><strong>Boutique Cloud</strong><span>Platform administration</span></div></div><div className="platform-console-user"><span><strong>{user.username}</strong><small>Platform administrator</small></span><button className="icon-button" onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} title="Toggle theme">{theme === 'light' ? <Moon size={18}/> : <Sun size={18}/>}</button><Button variant="ghost" icon={LogOut} onClick={logout}>Sign out</Button></div></header>
    <main className="platform-console-content"><Notice value={notice} clear={() => setNotice(null)}/><PlatformAdmin notice={setNotice}/></main>
  </div>
}

function Shell({ user, logout }) {
  const hasAccess = useCallback((id) => user.role === 'admin' || (user.role === 'owner' ? (user.permissions || []).includes(id) : user.role === 'viewer' ? VIEWER_FEATURES.includes(id) : (user.permissions || []).includes(id)), [user])
  const visibleNav = useMemo(() => NAV.filter(([id]) => id !== 'platform' && hasAccess(id)), [hasAccess])
...
function PlatformAdmin({ notice }) {
  const [data,setData]=useState({customers:[],totals:{}}),[search,setSearch]=useState(''),[busy,setBusy]=useState('')
  const load=useCallback(()=>request('/platform/customers').then(setData).catch((e)=>notice({type:'error',text:e.message})),[notice])
  useEffect(()=>{load()},[load])
  async function toggle(customer){setBusy(customer.id);try{await request(`/platform/customers/${customer.id}`,{method:'PATCH',body:JSON.stringify({active:!customer.active})});notice({text:`${customer.name} ${customer.active?'suspended':'reactivated'}.`});await load()}catch(e){notice({type:'error',text:e.message})}finally{setBusy('')}}
  const query=search.trim().toLocaleLowerCase(),customers=data.customers.filter((row)=>!query||[row.name,row.owner?.username,row.profile?.full_name,row.profile?.email,row.profile?.phone,row.profile?.organization_type,row.profile?.city,row.profile?.country].some((value)=>String(value||'').toLocaleLowerCase().includes(query)))
  return <><PageHead eyebrow="Private platform administration" title="Customer accounts" action={<Button variant="secondary" icon={RefreshCw} onClick={load}>Refresh</Button>}/>
    <Card className="platform-admin-banner"><span><ShieldCheck/></span><div><h2>Platform owner console</h2><p>Only your environment administrator account can access this page. Customer workspaces and their business records remain isolated from one another.</p></div></Card>
    <div className="metrics-grid"><Metric label="Registered workspaces" value={data.totals.workspaces||0} hint={`${data.totals.active||0} active`} icon={Building2}/><Metric label="Customer users" value={data.totals.users||0} hint="Workspace owners and team" icon={UsersRound} tone="green"/><Metric label="Recorded sales" value={data.totals.sales||0} hint={money(data.totals.revenue)} icon={ShoppingBag} tone="violet"/><Metric label="Estimated data usage" value={fileSize(data.totals.storage_bytes)} hint="MongoDB document estimate" icon={Database} tone="amber"/></div>
    <Card className="toolbar"><div className="search-box"><Search/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search organisation, owner, email, phone or location…"/></div><span>{customers.length} customer workspace{customers.length===1?'':'s'}</span></Card>
    {customers.length?<div className="platform-customer-grid">{customers.map((customer)=>{const profile=customer.profile||{},owner=customer.owner||{};return <Card className="platform-customer" key={customer.id}>
      <div className="platform-customer-head"><div className="request-avatar">{customer.name?.slice(0,1).toUpperCase()}</div><div><h2>{customer.name}</h2><p>{profile.organization_type||'Business workspace'} · {customer.plan} plan</p></div><span className={`badge ${customer.active?'green':'red'}`}>{customer.active?'Active':'Suspended'}</span></div>
      <div className="platform-owner"><span className="eyebrow">Owner details</span><div><strong>{profile.full_name||owner.username}</strong><small>@{owner.username}</small></div><p><Mail/>{profile.email||'No email'}</p><p><Phone/>{profile.phone||'No phone'}</p><p><MapPin/>{[profile.city,profile.state,profile.country].filter(Boolean).join(', ')||'No location'}</p></div>
      <div className="platform-profile"><span><strong>Owner role</strong>{profile.job_title||'Not provided'}</span><span><strong>Declared team</strong>{profile.team_size||'Not provided'}</span><span><strong>Website</strong>{profile.website||'Not provided'}</span><span><strong>Discovery</strong>{profile.how_heard||'Not provided'}</span></div>
      <div className="platform-stats"><span>Sales<strong>{customer.sales_count}</strong></span><span>Customers<strong>{customer.customer_count}</strong></span><span>Revenue<strong>{money(customer.revenue)}</strong></span><span>Pending<strong>{money(customer.pending)}</strong></span><span>Bills<strong>{customer.bills_count}</strong></span><span>Storage<strong>{fileSize(customer.storage_bytes)}</strong></span></div>
      <div className="platform-meta"><span><strong>Signup</strong>{customer.signup_method} · {dateLabel(customer.created_at)}</span><span><strong>Last login</strong>{dateLabel(customer.last_login_at)}</span><span><strong>Activity</strong>{dateLabel(customer.last_activity_at)}</span><span><strong>Logins</strong>{customer.login_count}</span><span><strong>Notes</strong>{customer.notes_count}</span><span><strong>Team</strong>{customer.team_members}</span></div>
      {profile.use_case&&<p className="platform-use"><strong>Use case</strong>{profile.use_case}</p>}
      <div className="platform-actions"><Button variant={customer.active?'danger':'secondary'} disabled={busy===customer.id} onClick={()=>toggle(customer)}>{busy===customer.id?'Updating…':customer.active?'Suspend workspace':'Reactivate workspace'}</Button></div>
    </Card>})}</div>:<Empty icon={Building2} title="No customer workspaces yet" copy="Public signups will appear here immediately."/>}
  </>
}
...
function PaymentModal({ row, close, save }) { const [form, setForm] = useState({ amount: row.pending_amount, date: today(), method: 'UPI', received_by: '' }); return <div className="modal-backdrop"><div className="modal"><button className="modal-close" onClick={close}><X /></button><span className="eyebrow">Collect payment</span><h2>{row.customer_name}</h2><p>Pending balance: <strong>{money(row.pending_amount)}</strong></p><div className="form-grid two"><Field label="Amount" type="number" max={row.pending_amount} value={form.amount} onChange={(e) => setForm({...form, amount:e.target.value})}/><Field label="Date" type="date" value={form.date} onChange={(e) => setForm({...form, date:e.target.value})}/><Field label="Method" as="select" value={form.method} onChange={(e) => setForm({...form, method:e.target.value})}>{PAYMENT_METHODS.slice(0,4).map(x=><option key={x}>{x}</option>)}</Field><Field label="Received by" value={form.received_by} onChange={(e) => setForm({...form, received_by:e.target.value})} placeholder="Defaults to signed-in user"/></div><Button onClick={() => save(form)} icon={CircleDollarSign}>Record collection</Button></div></div> }
...
function Billing({ sales, notice, readOnly, currentUser }) {
...
const blankSmtp = () => ({ provider:'gmail',enabled:true,host:'smtp.gmail.com',port:465,secure:true,user:'',password:'',from_name:'Boutique Cloud',from_email:'',reply_to:'',test_to:'',password_configured:false })
function Technical({sales,notes,currentUser,notice}) {
  ...
  const canManageSmtp=currentUser.source==='personal_boutique'
  useEffect(()=>{request('/health').then(setHealth);if(canManageSmtp)request('/smtp').then((row)=>setSmtp({...blankSmtp(),...row,password:'',test_to:row.from_email||row.user||''})).catch((e)=>notice({type:'error',text:e.message}))},[canManageSmtp,notice])
...
function IAM({ notice, currentUser }) {
  ...
  const editingUser = users.find((user) => user.id === editing), locked = ['environment', 'personal_boutique'].includes(editingUser?.source)
  return <><PageHead eyebrow="Identity & access management" title="IAM" action={<Button icon={Plus} onClick={reset}>New user</Button>} />
    <Card className="security-banner"><span><UserCog/></span><div><h2>Vinay Admin team access</h2><p>Create Custom or Viewer identities for this boutique. Every account and permission shown here belongs only to the Vinay Admin workspace.</p></div></Card>
...
function Backup({reload,notice}){const[file,setFile]=useState(null);async function download(){const token=localStorage.getItem('boutique_token');const r=await fetch('/api/backup',{headers:{Authorization:`Bearer ${token}`}});const blob=await r.blob(),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`boutique-backup-${today()}.json`;a.click();URL.revokeObjectURL(url);notice({text:'Workspace backup downloaded.'})}async function restore(){if(!file||!confirm('Insert all business records from this backup? Duplicates may result.'))return;try{const data=JSON.parse(await file.text());const r=await request('/restore',{method:'POST',body:JSON.stringify(data)});notice({text:`Restore complete: ${r.inserted} records inserted.`});reload()}catch(e){notice({type:'error',text:e.message})}}return <><PageHead eyebrow="Data resilience" title="Backup & restore" /><div className="backup-grid"><Card className="backup-card"><span className="backup-icon blue"><CloudDownload/></span><h2>Download workspace checkpoint</h2><p>Export only this boutique’s sales, work notes, bill history and saved passbook vendors.</p><Button icon={CloudDownload} onClick={download}>Download backup</Button></Card><Card className="backup-card"><span className="backup-icon green"><Upload/></span><h2>Restore workspace checkpoint</h2><p>Insert business records into the Vinay Admin workspace without affecting other customers.</p><input type="file" accept="application/json" onChange={e=>setFile(e.target.files[0])}/><Button variant="secondary" icon={ArchiveRestore} disabled={!file} onClick={restore}>Restore backup</Button></Card></div><Card className="env-help danger-help"><AlertTriangle/><div><h3>Restore is additive</h3><p>Existing records are preserved. Restoring the same checkpoint more than once can create duplicates.</p></div></Card></>}
...
export default function App() {
  ...
  return user ? (user.platform_admin ? <PlatformConsole user={user} logout={logout}/> : <Shell user={user} logout={logout}/>) : <PublicHome onLogin={setUser} />
}
