...
function Technical({sales,notes,currentUser,notice}) {
  const [health,setHealth]=useState(null),[smtp,setSmtp]=useState(blankSmtp()),[smtpBusy,setSmtpBusy]=useState(false),[testing,setTesting]=useState(false)
  ...
  async function saveSmtp(e){e.preventDefault();setSmtpBusy(true);try{const {test_to:_testTo,password_configured:_configured,...payload}=smtp;const saved=await request('/smtp',{method:'PUT',body:JSON.stringify(payload)});setSmtp((old)=>({...old,...saved,password:'',test_to:old.test_to||saved.from_email}));notice({text:'SMTP email settings saved securely.'})}catch(e){notice({type:'error',text:e.message})}finally{setSmtpBusy(false)}}
  async function testSmtp(){setTesting(true);try{await request('/smtp/test',{method:'POST',body:JSON.stringify({to:smtp.test_to}),timeout:30000});notice({text:`Test email sent to ${smtp.test_to}.`})}catch(e){notice({type:'error',text:e.message})}finally{setTesting(false)}}
  return <><PageHead eyebrow="System administration" title="Technical overview" />
    ...
    <div className="technical-layout"><Card className="technical-card"><div className="card-title"><div><h2>Data collections</h2><p>Live records available to this workspace</p></div></div>{collections.map(([n,v])=><div className="collection-row" key={n}><span><Database/>{n}</span><strong>{v} documents</strong></div>)}</Card>
      {currentUser.role==='admin'?<Card className="smtp-card"><div className="card-title"><div><h2>Outgoing email · SMTP</h2><p>Connect Gmail or another provider to send workspace email</p></div><span className={`badge ${smtp.password_configured?'green':'amber'}`}>{smtp.password_configured?'Configured':'Setup required'}</span></div>
