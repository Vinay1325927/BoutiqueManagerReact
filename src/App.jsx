...
import {
  ...
  Eye, Fingerprint, Home, IndianRupee, KeyRound, LockKeyhole, LogIn, LogOut, Menu, Moon, NotebookPen, PackagePlus, PencilLine, Plus,
...
function Shell({ user, logout }) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const inventoryAccess = ['inventory', 'vendors', 'ai', 'technical'].some(hasAccess)
      const notesAccess = ['notes', 'ai', 'technical'].some(hasAccess)
      const [s, i, n] = await Promise.all([salesAccess ? request('/sales') : [], inventoryAccess ? request('/inventory') : [], notesAccess ? request('/notes') : []])
      setSales(s); setInventory(i); setNotes(n)
  ...
  const context = { sales, inventory, notes, reload, notice: setNotice, loading, readOnly }
  const pages = { dashboard: <Dashboard {...context} go={navigate} canAccess={hasAccess} />, 'add-sale': <AddSale {...context} />, review: <Review {...context} />, update: <UpdateSale {...context} />, customers: <Customers {...context} />, vendors: <Vendors {...context} />, analytics: <Analytics {...context} />, reminders: <Reminders {...context} />, bill: <Billing {...context} />, inventory: <Inventory {...context} />, passbook: <Passbook {...context} />, notes: <Notes {...context} />, ai: <AI />, technical: <Technical {...context} />, iam: <IAM notice={setNotice} currentUser={user} />, security: <Security notice={setNotice} />, backup: <Backup {...context} /> }
...
function Inventory({ inventory, reload, notice, readOnly }) {
...
function Security({ notice }) {
  const [devices, setDevices] = useState([]), [users, setUsers] = useState([]), [userId, setUserId] = useState(''), [pemFile, setPemFile] = useState(null), [busy, setBusy] = useState(false)
  ...
  async function enroll(e) {
    e.preventDefault(); if (!userId || !pemFile) return; setBusy(true)
    try { await request(`/iam/users/${userId}/pem`, { method: 'PUT', body: JSON.stringify({ filename: pemFile.name, pem: await pemFile.text() }) }); notice({ text: 'Public PEM key enrolled. The matching private key can now be used at login.' }); setPemFile(null); await load() } catch (e) { notice({ type: 'error', text: e.message }) } finally { setBusy(false) }
  }
  ...
  return <><PageHead eyebrow="Access control" title="Security & devices" /><Card className="security-banner"><span><ShieldCheck/></span><div><h2>Authentication security</h2><p>Enroll RSA public keys for PEM login and revoke browser sessions. Private PEM keys must remain only on each user’s device.</p></div></Card><div className="security-grid"><Card className="pem-card"><div className="card-title"><div><h2>PEM sign-in</h2><p>Register a public key or certificate for a workspace user</p></div><span className="pem-icon"><Fingerprint/></span></div><form onSubmit={enroll}><Field label="IAM user" as="select" value={userId} onChange={(e) => { setUserId(e.target.value); setPemFile(null) }}>{users.map((user) => <option value={user.id} key={user.id}>{user.username} — {ROLE_LABELS[user.role]}</option>)}</Field><label className="pem-picker"><span>Public PEM file</span><input type="file" accept=".pem,.crt,.cer,application/x-pem-file" onChange={(e) => setPemFile(e.target.files[0] || null)} /><small>{pemFile ? pemFile.name : 'Accepted: BEGIN PUBLIC KEY or BEGIN CERTIFICATE. Never upload a private key here.'}</small></label>{selected?.pem?.enabled && <div className="fingerprint"><span>Registered key</span><strong>{selected.pem.filename}</strong><code>{selected.pem.fingerprint}</code><small>Enrolled {dateLabel(selected.pem.enrolled_at)}</small></div>}<div className="form-actions">{selected?.pem?.enabled && <Button type="button" variant="danger" onClick={removePem}>Remove key</Button>}<Button icon={KeyRound} disabled={!pemFile || !userId || busy}>{busy ? 'Registering…' : selected?.pem?.enabled ? 'Replace public key' : 'Register public key'}</Button></div></form></Card><Card className="pem-help"><span className="pem-icon"><LockKeyhole/></span><h2>How PEM login works</h2><ol><li>Generate an RSA key pair for the user.</li><li>Register only the public PEM here.</li><li>The user selects their private PKCS#8 PEM on the login screen.</li><li>The browser signs a one-time challenge locally; the private file is never uploaded.</li></ol></Card></div><div className="section-divider"><span>Authenticated device sessions</span></div><Card className="table-card">{devices.length?<div className="table-wrap"><table><thead><tr><th>User</th><th>Method</th><th>Last login</th><th>Status</th><th/></tr></thead><tbody>{devices.map(x=><tr key={x.id}><td><strong>{x.username}</strong><small>{x.user_agent?.slice(0,55)}</small></td><td>{x.login_method}</td><td>{dateLabel(x.last_login_at)}</td><td><span className={`badge ${x.active?'green':'red'}`}>{x.active?'Active':'Revoked'}</span></td><td><Button variant="tiny" onClick={()=>toggle(x)}>{x.active?'Revoke':'Restore'}</Button></td></tr>)}</tbody></table></div>:<Empty icon={ShieldCheck} title="No devices recorded"/>}</Card></>
}
